...
**fix: 知识图谱与AI面试模块关键Bug修复**

#### 修复内容：

1.  **知识图谱 Back 按钮定位修复 (Knowledge Graph Back Button)**
    *   **问题**: Back 按钮位置不当，遮挡左侧课程菜单栏。
    *   **修复**: 将 Back 按钮重新定位至图谱可视化区域（灰色背景）的左上角，避免与侧边栏重叠。
    *   **技术细节**: 按钮现在位于 `bg-slate-50` 容器内部但在 transform 容器外部，确保在缩放/平移时保持固定位置。

2.  **AI 面试模拟器白屏问题修复 (Interview Simulator White Screen)**
    *   **问题**: 删除最后一道题目时，应用显示白屏崩溃。
    *   **根本原因**: 组件在多处直接访问 `qs[idx]` 而未检查数组元素是否存在；Firestore 异步更新导致渲染时数组已空但索引未调整。
    *   **修复方案**:
        *   添加 `useEffect` 监听题目数组变化，自动调整索引至有效范围。
        *   引入 `currentQ` 和 `hasQuestions` 安全变量，替换所有直接的 `qs[idx]` 访问。
        *   实现空状态友好提示："暂无题目，点击下方'AI 出题'按钮生成新题目"。
    *   **影响**: 现在删除任意题目（包括最后一题）都能平滑过渡，不会出现崩溃或白屏。
